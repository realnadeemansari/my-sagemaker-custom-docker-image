...
def lambda_handler(event, context):
    ...
    try:
        ...
        print("s3://{}/{}/".format(bucket, folders[0] + "/" + folders[1] + "/" + folders[2]))
        if prefix == "data":
            data_location = "s3://{}/{}/".format(bucket, folders[0] + "/" + folders[1] + "/" + folders[2])
            print("Training started")
            result = sm_client.create_training_job(TrainingJobName="prophet-model-" + folders[2] + "-auto-training-" + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'),
